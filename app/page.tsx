...
export default function Home() {
  ...
  async function startFlow() {
    const r = await fetch("/api/proxy/start", {
      ...
      body: JSON.stringify({ flowId: "jas" }),
    });
    ...
    <main className="min-h-screen bg-black text-white font-mono px-6 py-12">
      <div className="max-w-2xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-lg">jas</h1>
          <button onClick={startFlow} className="text-xs text-neutral-600 hover:text-white border border-neutral-800 px-3 py-1 rounded">+ new run</button>
        </div>
