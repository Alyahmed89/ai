  ...
  try {
    ...
    if (!id) {
      return NextResponse.json(
        { success: false, error: 'Missing flow step ID', statusCode: 400 },
        { status: 400 }
      );
    }
    ...
    if (results.length === 0) {
      return NextResponse.json(
        { success: false, error: 'Flow step not found', statusCode: 404 },
        { status: 404 }
      );
    }
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to fetch flow step input', statusCode: 500 },
      { status: 500 }
    );
  }
}