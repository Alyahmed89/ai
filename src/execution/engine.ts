...
export async function runStep(stepKnowledge: any, flowExecutionId: string, flowExec: any): Promise<any> {
  ...
  if (tags.includes('action')) {
    ...
    let ep = ctx;
    if (endpointId) {
      const { data: epData } = await getSupabase()
        .from('knowledge')
        .select('context')
        .eq('id', endpointId)
        .single();
    ...
    for (const [k, v] of Object.entries(rawPayload)) {
      payload[k] = typeof v === 'string' ? resolveVariables(v, context) : v;
    }
    const baseUrl = process.env.BACKEND_URL || 'https://ai.anyapp.cfd';
    const fullUrl = url.startsWith('http') ? url : `${baseUrl}${url}`;
    const actionResp = await fetch(fullUrl, {
      method,
      headers: { 'Content-Type': 'application/json' },
    ...
    for (const [key, value] of Object.entries(output)) {
      await storeMemory(flowExecutionId, key, typeof value === 'string' ? value : JSON.stringify(value), 'flow');
    }
    await getSupabase().from('executions').update({
      status: 'completed', output, updated_at: new Date().toISOString(),
